...
void AfficherCommande() {
    ...
    for (int i = 0; i < getGame()->nbCommandes; i++) {
        ...
        rotate_scaled_sprite(getCorrectBuffer(), getGraphic()->textures.ticket, xLogo, 340,
                             getGame()->commandes[i].angle, ftofix((float) 110 / getGraphic()->textures.ticket->w));
        ...
        if (strcmp(getGame()->commandes[i].recette.nom, "Hamburger") == 0) {
            ...
            stretch_sprite(getCorrectBuffer(), getGraphic()->textures.pizza, 37 + xLogo, 408, 38, 38);
        }
        ...
        for (int j = 0; j < getGame()->commandes[i].recette.nbIngredients; j++) {
            if (getGame()->commandes[i].recette.ingredients[j].nom == SALADE) {
                stretch_sprite(getCorrectBuffer(), getGraphic()->textures.Laitue,
                               coosX[j] + i * (LargeurRectangle + EspaceHorizontalEntreLesRectangle), coosY[j], 18, 18);
            } else if (getGame()->commandes[i].recette.ingredients[j].nom == PAIN) {
                stretch_sprite(getCorrectBuffer(), getGraphic()->textures.Pain,
                               coosX[j] + i * (LargeurRectangle + EspaceHorizontalEntreLesRectangle), coosY[j], 18, 18);
            } else if (getGame()->commandes[i].recette.ingredients[j].nom == TOMATE) {
                stretch_sprite(getCorrectBuffer(), getGraphic()->textures.tomate,
                               coosX[j] + i * (LargeurRectangle + EspaceHorizontalEntreLesRectangle), coosY[j], 18, 18);
            } else if (getGame()->commandes[i].recette.ingredients[j].nom == STEAK) {
                stretch_sprite(getCorrectBuffer(), getGraphic()->textures.steak,
                               coosX[j] + i * (LargeurRectangle + EspaceHorizontalEntreLesRectangle), coosY[j], 18, 18);
            } else if (getGame()->commandes[i].recette.ingredients[j].nom == OEUF) {
                stretch_sprite(getCorrectBuffer(), getGraphic()->textures.Oeuf,
                               coosX[j] + i * (LargeurRectangle + EspaceHorizontalEntreLesRectangle), coosY[j], 18, 18);
            } else if (getGame()->commandes[i].recette.ingredients[j].nom == POTATO) {
                stretch_sprite(getCorrectBuffer(), getGraphic()->textures.PommeDeTerre,
                               coosX[j] + i * (LargeurRectangle + EspaceHorizontalEntreLesRectangle), coosY[j], 18, 18);
            } else if (getGame()->commandes[i].recette.ingredients[j].nom == FROMAGE) {
                stretch_sprite(getCorrectBuffer(), getGraphic()->textures.Fromage,
                               coosX[j] + i * (LargeurRectangle + EspaceHorizontalEntreLesRectangle), coosY[j], 18, 18);
            }
        }
    }
